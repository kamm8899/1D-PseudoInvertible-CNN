...
# ====================== LOAD DATA ======================
# Use unnormalized data so power level is available as a discriminating feature
...
train_noise_raw = torch.load("spectrum_data/train_noise_raw.pt", weights_only=False)
...
def compute_mse(model, data):
    """Compute per-sample MSE = mean((x - recon)^2). Higher MSE → anomaly."""
    mses = []
    with torch.no_grad():
...
print("Computing MSE on training noise (H0) for threshold estimation...")
train_mse = compute_mse(model_cae, train_noise)

mu_e, sigma_e = np.mean(train_mse), np.std(train_mse)
print(f"CAE H0 MSE → mean = {mu_e:.6f},  std = {sigma_e:.6f}")

# ====================== NEYMAN-PEARSON THRESHOLD γ ======================
# Upper-tail: MSE > γ → anomaly (signal has higher reconstruction error than noise)
# P_fa = P(MSE > γ | H0) = 1 - Φ((γ - μ) / σ) → γ = μ + Φ⁻¹(1 - P_fa)·σ
target_pfa = 0.01
gamma = mu_e + norm.ppf(1 - target_pfa) * sigma_e
print(f"Target P_fa = {target_pfa} → γ = {gamma:.6f}")

# ====================== TEST SET EVALUATION ======================
print("\nComputing MSE scores on test set...")
mse_cae = compute_mse(model_cae, test_data)

# ROC / AUC — higher MSE = anomaly
fpr_cae, tpr_cae, thresholds_cae = roc_curve(test_labels, mse_cae)
...
print(f"\n=== FINAL RESULTS (CAE — MSE Detection) ===")
print(f"CAE  AUC: {auc_cae:.4f}  γ = {gamma:.6f}  params = {param_cae:,}")
...
with open("spectrum_data/evaluation_results_cae.txt", "w") as f:
    f.write("=== EVALUATION RESULTS (CAE — MSE Detection) ===\n")
    f.write(f"Date: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    f.write(f"CAE AUC: {auc_cae:.4f}\n")
    f.write(f"CAE γ (P_fa={target_pfa}): {gamma:.6f}\n")
...
plt.title('ROC Curve - Modulation-Agnostic Anomaly Detection (MSE) — CAE')
plt.legend()
plt.grid(True)
...
print(f"\n{'='*60}")
print(f"TPR BY SNR RANGE (CAE MSE, γ P_fa={target_pfa})")
...
for snr_low, snr_high in [(-10, -5), (-5, 0), (0, 5), (5, 10)]:
    ...
    if mask.sum() == 0:
        continue
    fpr_s, tpr_s, _ = roc_curve(test_labels[mask], mse_cae[mask])
...
print(f"\n{'='*60}")
print(f"TPR BY MODULATION TYPE (CAE MSE, γ P_fa={target_pfa})")
...
target_pfa = 0.05
gamma = mu_e + norm.ppf(1 - target_pfa) * sigma_e
print(f"\nTarget P_fa = {target_pfa} → γ = {gamma:.6f}")
...
beta_cae = 1 - mse_cae / (mse_cae.max() + 1e-8)   # placeholder — kept for legacy plot label
plt.figure(figsize=(8,6))
plt.hist(mse_cae[test_labels==0], bins=50, alpha=0.5, label='CAE Noise (H0)')
plt.hist(mse_cae[test_labels==1], bins=50, alpha=0.5, label='CAE Signal (H1)')
plt.axvline(gamma, color='red', linestyle='--', label=f'CAE γ (P_fa={target_pfa})')
plt.xlabel('MSE Score')
plt.ylabel('Frequency')
plt.title('Distribution of MSE Scores - CAE (β plot replaced)')
plt.legend()
plt.grid(True)
plt.savefig(out_dir / "beta_distribution_cae.png", dpi=300, bbox_inches='tight')
plt.close()

# ====================== MSE DISTRIBUTIONS AT -6, 0, +6 dB ======================
target_pfa = 0.01
gamma = mu_e + norm.ppf(1 - target_pfa) * sigma_e
...
for snr_val in [-6, 0, 6]:
    ...
    fig.suptitle(f'CAE MSE Distribution at SNR = {snr_val:+d} dB  (Pfa={target_pfa})', fontsize=13)

    h0 = mask & (test_labels == 0)
    h1 = mask & (test_labels == 1)

    ax.hist(mse_cae[h0], bins=40, alpha=0.6, color='steelblue',  label='Noise (H0)')
    ax.hist(mse_cae[h1], bins=40, alpha=0.6, color='darkorange', label='Signal (H1)')
    ax.axvline(gamma, color='red', linestyle='--', label='γ threshold')
    ax.set_title('CAE — MSE Score')
    ax.set_xlabel('MSE')
...
for snr_db in snr_points:
    ...
    pd_c = float(np.mean(mse_cae[sig_mask] > gamma)) if sig_mask.sum() > 0 else np.nan
